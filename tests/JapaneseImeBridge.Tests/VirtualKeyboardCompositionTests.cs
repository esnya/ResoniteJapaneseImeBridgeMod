...
public sealed class VirtualKeyboardCompositionTests
{
    ...
    {
        var assembly = typeof(JapaneseImeBridgeController).Assembly;

        Assert.Null(assembly.GetType("JapaneseImeBridge.Composition.ITextCommitter", throwOnError: false));
        Assert.Null(assembly.GetType("JapaneseImeBridge.Composition.TextEditorFocusTracker", throwOnError: false));
    }
    ...
    public void VirtualKeyInputMapsControlKeysBeforeText()
    {
        ...
        Assert.True(mapped);
        Assert.Equal(ImeBackendKey.Space, request.Key);
    ...
    public void VirtualKeyInputMapsSegmentKeysOnlyDuringComposition()
    {
        ...
        Assert.True(mapped);
        Assert.Equal(ImeBackendKey.Left, request.Key);
    }
    ...
    public void VirtualKeyInputMapsSegmentWidthKeysOnlyDuringComposition()
    {
        ...
        Assert.Equal(ImeBackendKey.SegmentWidthShrink, shrinkRequest.Key);
        Assert.Equal(ImeBackendKey.SegmentWidthExpand, expandRequest.Key);
    }
    ...
    {
        var settings = new JapaneseImeBridgeSettings(
            Enabled: true,
            string.Empty,
