...
internal static class MozcIpcCommandCodec
{
    ...
    public static byte[] SendKey(ulong sessionId, MozcBridgeRequest request) =>
        WriteInput(input =>
        {
    ...
    private static uint ToSpecialKey(MozcBridgeKey key) =>
        key switch
        {
            MozcBridgeKey.Space => 4,
            MozcBridgeKey.Enter => 5,
            MozcBridgeKey.Escape => 10,
            MozcBridgeKey.Backspace => 12,
            MozcBridgeKey.Left or MozcBridgeKey.SegmentWidthShrink => 6,
            MozcBridgeKey.Right or MozcBridgeKey.SegmentWidthExpand => 7,
            MozcBridgeKey.Up => 8,
            MozcBridgeKey.Down => 9,
            _ => 0,
        };

    private static bool RequiresShiftModifier(MozcBridgeKey key) =>
        key is MozcBridgeKey.SegmentWidthShrink or MozcBridgeKey.SegmentWidthExpand;
